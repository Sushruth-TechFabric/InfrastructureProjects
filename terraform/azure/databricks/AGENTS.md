...
3. **Naming convention is an API contract:**
   `{type}-{workload}-{env}-{region}-{instance}` (abbreviate where Azure limits
...
5. **Networking:** two delegated Databricks subnets (`/26`+, prefer `/23`–`/24` in
   prod) + a separate private-endpoint subnet; VNet `/16`–`/24`. Forced tunneling
   (`0.0.0.0/0` → hub firewall) with an egress allowlist. Private endpoints +
   linked Private DNS zones for every PaaS service (forgetting the DNS link is the
   classic silent failure).
6. **Databricks security:** SCC on (no public IP); **back-end** Private Link for the
   compute plane. The **front-end is internet-reachable, gated by Entra ID + workspace IP
   access lists** (no front-end Private Link, no user VNet path). Data and secret planes
   stay private (storage/Key Vault public access **disabled**).
